...
        // Зелена зона: 0.8 - 1.3
        // Масштабування: 0.8 = -30 deg, 1.05 = 0 deg, 1.3 = +30 deg (змінено для кращого відображення)
        degree = (acwrValue - 1.05) * 120; // 0.25 діапазон, 30 градусів.
        status = 'Безпечна зона (Оптимально)';
        statusClass = 'status-safe';
    } else if (acwrValue < 0.8 && acwrValue >= 0.5) {
        // Жовта зона (Недовантаження): 0.5 - 0.8
        degree = -50 + (acwrValue - 0.5) * 100; // -50deg до -20deg (приблизно)
...
    
    // !!! ВИДАЛЕНО обробник RPE, який викликав помилку Cannot set properties of null
}
...
        statusMessage.className = 'status-box status-danger'; // Червоний статус для помилки
        return;
    }
...
    // Перевіряємо, чи вже є запис на цю дату, і оновлюємо його
...
    }
    
    statusMessage.className = 'status-box status-safe'; // Зелений статус для успіху
    
